...
| **GitHub 仓库** | https://github.com/_<你的用户名>_/cs599-project |
| **开源协议** | MIT License |
...
| 指标 | 计算方式 | 目标 | 实测（待运行后填入） |
| --- | --- | --- | --- |
| 召回率 | 确认捕获类数 / 6 | ≥ 80 % | _见 docs/eval/run-NN.json_ |
| 误报率 | Verifier 驳回数 / Reviewer 总产出 | ≤ 30 % | _见 docs/eval/run-NN.json_ |
| 修复成功率 | 修复后 `python -c "import sample_buggy"` 成功 / 总 confirmed | ≥ 70 % | _见 docs/eval/run-NN.json_ |
| 平均时延 | 端到端 wall-clock | ≤ 60 s/file | _见 docs/eval/run-NN.json_ |
...
默认 JSON：
```json
{"event":"reviewer_done","findings":7,"agent":"agent.reviewer","level":"info","timestamp":"2026-06-07T15:32:18"}
{"event":"verifier_done","total":7,"confirmed":4,"rejected":3,"agent":"agent.verifier","level":"info","timestamp":"2026-06-07T15:32:24"}
{"event":"fixer_applied","file":"sample_buggy.py","line":14,"result":"[OK] 已写入 sample_buggy.py（842 字节）","agent":"agent.fixer","level":"info","timestamp":"2026-06-07T15:32:31"}
```
